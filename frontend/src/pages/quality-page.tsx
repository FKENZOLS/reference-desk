...
const emptyState: QualityState = {
  reranker: { choice: "gte", label: "GTE", model: "Alibaba-NLP/gte-multilingual-reranker-base" },
  summary: {
    total: 0,
    counts: { relevant: 0, wrong_passage: 0, wrong_document: 0, no_relevant_result: 0, expected_passage: 0, ambiguous: 0 },
    benchmark_cases: 0,
    answerable_cases: 0,
...
const judgmentLabels: Record<Feedback["judgment"], string> = {
  ...
  expected_passage: "Expected passage",
  ambiguous: "Ambiguous",
}
...
export function QualityPage() {
  ...
  const [qualityReranker, setQualityReranker] = useState<"gte" | "bge">("gte")

  async function refresh() {
    try { setState(await api<QualityState>(`/quality/api/state?reranker_choice=${qualityReranker}`)) }
    catch (error) { toast.error(error instanceof Error ? error.message : "Could not load quality data") }
    finally { setLoading(false) }
  }
  useEffect(() => { setLoading(true); void refresh() }, [qualityReranker])

  const calibration = state.summary.calibration
  const rejectedCount = ["wrong_passage", "wrong_document", "no_relevant_result"]
    .reduce((total, key) => total + (state.summary.counts[key] || 0), 0)
  ...
  async function calibrate() {
    try {
      const value = await api<Calibration>(`/quality/api/calibrate?reranker_choice=${qualityReranker}`, { method: "POST" })
  ...
  async function toggleGate(enabled: boolean) {
    try {
      const value = await api<Calibration>("/quality/api/calibration", jsonRequest("PATCH", { enabled, reranker_choice: qualityReranker }))
      setState((current) => ({ ...current, summary: { ...current.summary, calibration: value } }))
  ...
  return (
    <div>
      <PageHeader title="Reference quality" actions={<>
        <div className="grid h-9 grid-cols-2 rounded-lg border bg-background" role="group" aria-label="Quality reranker">
          {(["gte", "bge"] as const).map((choice) => <button key={choice} type="button" onClick={() => setQualityReranker(choice)} className={qualityReranker === choice ? "rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground" : "rounded-md px-3 text-xs text-muted-foreground hover:bg-accent"}>{choice.toUpperCase()}</button>)}
        </div>
        <Button variant="outline" asChild><a href={`/quality/export/benchmark?reranker_choice=${qualityReranker}`}><Download /> Export benchmark</a></Button>
      ...
      <Card className="bg-card/75"><CardContent className="p-0">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4">
          ...
          <div className="flex flex-wrap gap-1">{(["all", "relevant", "wrong_passage", "wrong_document", "no_relevant_result", "expected_passage", "ambiguous"] as const).map((value) => <Button key={value} size="sm" variant={filter === value ? "secondary" : "ghost"} onClick={() => setFilter(value)}>{value === "all" ? "All" : judgmentLabels[value]}</Button>)}</div>
        </div>
        {loading ? <div className="p-10 text-center text-muted-foreground">Loading labels…</div> : feedback.length ? (
          <div className="overflow-x-auto"><table className="w-full border-collapse text-left"><thead><tr className="border-b text-[11px] uppercase tracking-wide text-muted-foreground"><th className="px-4 py-3 font-medium">Query</th><th className="px-4 py-3 font-medium">Judgment</th><th className="hidden px-4 py-3 font-medium md:table-cell">Source</th><th className="hidden px-4 py-3 font-medium lg:table-cell">Score</th><th className="hidden px-4 py-3 font-medium xl:table-cell">Updated</th><th className="px-4 py-3"></th></tr></thead><tbody>{feedback.map((item) => <tr key={item.id} className="border-b last:border-0 hover:bg-muted/20"><td className="max-w-md px-4 py-3"><p className="m-0 line-clamp-2 font-medium">{item.query}</p>{item.reason && <p className="m-0 line-clamp-1 text-xs text-muted-foreground">{item.reason}</p>}{item.section && <p className="m-0 truncate text-xs text-muted-foreground">{item.section}</p>}</td><td className="px-4 py-3"><Badge variant={judgmentVariants[item.judgment]}>{item.judgment === "relevant" || item.judgment === "expected_passage" ? <CheckCircle2 className="mr-1 size-3" /> : <XCircle className="mr-1 size-3" />}{judgmentLabels[item.judgment]}</Badge></td><td className="hidden max-w-xs px-4 py-3 md:table-cell"><p className="m-0 truncate text-sm">{item.document_title || item.source_id || item.expected_source_id || "—"}</p>{(item.page_start || item.expected_page) && <p className="m-0 text-xs text-muted-foreground">page {item.page_start || item.expected_page}</p>}</td><td className="hidden px-4 py-3 tabular-nums text-muted-foreground lg:table-cell">{item.rerank_logit == null ? "—" : item.rerank_logit.toFixed(3)}</td><td className="hidden px-4 py-3 text-xs text-muted-foreground xl:table-cell">{formatDate(item.updated_at || item.created_at)}</td><td className="px-4 py-3"><Button size="icon" variant="ghost" className="text-destructive" onClick={() => void removeFeedback(item.id)} aria-label="Remove label"><Trash2 /></Button></td></tr>)}</tbody></table></div>
