...
export function DocumentsPage() {
  ...
  async function allowIncompleteQuarantine(quarantineId: string) {
    if (!window.confirm("Index the usable passages even though one or more text pages are missing? Search results from this document may be incomplete.")) return
    try {
      await api(`/documents/api/quarantine/${encodeURIComponent(quarantineId)}/allow-incomplete`, { method: "POST" })
      toast.success("Document returned to the queue with a partial-index override. Apply changes to index it.")
      await refresh(true)
    } catch (error) { toast.error(error instanceof Error ? error.message : "Could not allow partial indexing") }
  }
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
                ...
                <tbody>{filtered.map((document) => <tr key={document.source_id} className="border-b last:border-0 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3"><div className="flex flex-wrap gap-1"><Badge variant={document.status === "indexed" ? "default" : document.status === "failed" ? "destructive" : "secondary"}>{document.status}</Badge>{Boolean(document.index_warnings?.length) && <Badge variant="secondary">partial index</Badge>}</div>{document.state_updated_at && <p className="mb-0 mt-1 text-[10px] text-muted-foreground">{formatDate(document.state_updated_at)}</p>}{Boolean(document.index_warnings?.length) && <p className="mb-0 mt-1 max-w-64 text-[10px] text-amber-200" title={document.index_warnings?.join(" ")}>{document.index_warnings?.[0]}</p>}{Boolean(document.state_history?.length) && <details className="mt-1"><summary className="cursor-pointer text-[10px] text-muted-foreground">Lifecycle</summary><div className="mt-1 w-64 space-y-1 rounded-md border bg-popover p-2">{document.state_history?.slice().reverse().slice(0, 6).map((event, index) => <div key={`${event.at}-${index}`} className="text-[10px]"><span className="font-medium">{event.from ? `${event.from} → ` : ""}{event.to}</span><span className="text-muted-foreground"> · {formatDate(event.at)}</span>{event.reason && <p className="m-0 text-muted-foreground">{event.reason}</p>}</div>)}</div></details>}</td>
